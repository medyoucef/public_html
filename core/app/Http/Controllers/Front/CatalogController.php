...
// dd($request->all());
        ...
        $city = $request->has('city') ? ( !empty($request->city) ? Brand::where('id',$request->city)->firstOrFail() : null ) : null;
...
$items = Item::with('category')
        ->when($category, function ($query, $category) {
           ...
           ->when($city, function ($query, $city) {
            return $query->where('brand_id', $city->id);
        })
    ...
    public function suggestSearch(Request $request)
    {
        ...
        ->when($search, function ($query, $search) {
            return $query->where('name', 'like', '%' . $search . '%')->orderby('id','desc')->take(100);
        })
        ->when($category, function ($query, $category) {
            return $query->where('category_id', $category->id);
        })
