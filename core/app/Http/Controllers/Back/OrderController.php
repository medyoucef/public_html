...
class OrderController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($request->type){
            if($request->start_date && $request->end_date){
                ...
                $datas = Customer::latest('id')->whereOrderStatus($request->type)->whereDate('created_at','>=',$start_date)->whereDate('created_at','<=',$end_date)->get();
            }else{
                $datas = Customer::latest('id')->whereOrderStatus($request->type)->get();
            }
            ...
            if($request->start_date && $request->end_date){
                $datas = $start_date = Carbon::parse($request->start_date);
                $end_date = Carbon::parse($request->end_date);
                $datas = Customer::latest('id')->whereDate('created_at','>=',$start_date)->whereDate('created_at','<=',$end_date)->get();
            }else{
                $datas = Customer::latest('id')->get();
            }
        }
   ...
   $order = Customer::findOrfail($id);
       ...
       public function printContract($id)
    {
             $order = Customer::findOrfail($id);
        $cart = json_decode($order->cart, true);
        return view('back.order.contract-of-sale',compact('order','cart'));
    }
    /**
